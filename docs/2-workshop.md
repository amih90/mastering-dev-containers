...
1. Create a **devcontainer.json** file by opening the [Command Palette](https://code.visualstudio.com/docs/getstarted/userinterface#_command-palette) with `Ctrl` + `Shift` + `P` and selecting **Dev Containers: Add Development Container Configuration Files...**.
...
1. Select **3.11-bullseye** to match Lihi's required Python version.
    ...
    ![Add Dev Container Configuration Files - Python version](../assets/vscode-extension-add-dev-container-configuration-files-python311-bullseye.png)
...
1. Navigate to the **devcontainer.json** file via File Explorer `Ctrl` + `shift` + `E` or using the [Command Palette](https://code.visualstudio.com/docs/getstarted/userinterface#_command-palette) with `Ctrl` + `Shift` + `P`, and selecting **Dev Containers: Open Container Configuration File**, and set up the following configurations:
    ...
    1.  The **customizations** section allows you to specify IDE settings, extensions, and other customizations that will be applied when the dev container is created and used. This ensures that the development environment inside the container is tailored to the project's needs, providing a consistent and productive experience for all developers working on the project, see more [Container specific settings](https://code.visualstudio.com/docs/devcontainers/containers#_container-specific-settings).

        Update the **customizations.vscode** section to set the Python interpreter path and install the Python extension when working in Visual Studio Code.
    ...
    1.  The **portAttributes** property enables you to define how specific ports should be managed when forwarded from the container to the host, including setting labels, notifications, and other behaviors. By configuring portAttributes, you can control how the IDE handles port forwarding, making it easier to manage and identify forwarded ports in your development environment.

        Add the **portsAttributes** property with the desired port configurations to notify when Lihi's web application will be served.
        
        ```json
        "portsAttributes": {
            "8000": {
                "label": "Lihi's Web Application",
        ...
        > **Explanation**:
        > * **8000**: This is the port number that you want to configure. In this case, it is port 8000.
        >
        > * **label**: This is a human-readable label for the port. It helps you identify the purpose of the port. Here, it is labeled as "Lihi's Web Application".
        > * **onAutoForward**: This property defines the behavior when the port is automatically forwarded. The value "notify" means that IDE will show a notification when the port is forwarded.
...
1. **Validate**: After the container will be opened, VS Code will be mounted only with the webapp folder and the webapp requirements will be installed. Validate the web application has all the necessary dependencies installed by running and testing the endpoint:
    1.  Open a new terminal with `Ctrl` + `Shift` + ``` ` ``` and run the web server:
        ...
        Once the web server is up and running, the IDE will notify you

        ![Port Attributes](../assets/portsAttributes.png)
...
1. **Integrate Ollama Feature**: Ollama is a free and open-source tool that lets anyone run open LLMs locally on your system. It supports Linux, Windows, and macOS. It is a command-line interface (CLI) tool that lets you conveniently download LLMs and run it locally and privately.

    Enhance the project by adding the [Ollama](https://ollama.com/) feature, providing advanced tools and capabilities for Lihi's Generative AI LLMs development.

    ```json
    "ghcr.io/prulloac/devcontainer-features/ollama:1": {}
    ```

1. **Add Jupyter Notebook Extension**: Jupyter Notebooks are a powerful and dynamic tool used for interactively developing and presenting data science projects. They combine code, visualizations, narrative text, and other rich media into a single document, creating a cohesive and expressive workflow. This makes them an essential part of the data science workflow in companies and organizations worldwide.

    To enable Lihi with an interactive data analysis and model development, install the Jupyter Notebook extension in the dev container.
    ...
    With Jupyter Notebooks, Lihi will be able to interactively develop and test her Generative AI models. The notebooks will allow her to:

    - **Write and Execute Code**: Python code can be written and executed in real-time, with results seen immediately. This is particularly useful for experimenting with different model configurations and parameters.
    - **Visualize Data**: Rich visualizations can be created, enabling the creation of plots and charts that help understand the performance and behavior of models.
    - **Document Findings**: Findings and insights can be documented directly within the notebook, combining code, results, and narrative text in one place. This makes it easier to share work with colleagues and stakeholders.
    - **Iterate Quickly**: The interactive nature of Jupyter Notebooks allows for quick iteration on models, making adjustments and seeing the impact immediately.

    By integrating Jupyter Notebooks into her development environment, Lihi can streamline her workflow and enhance her productivity, making it easier to develop and refine her Generative AI models.
...
<div width=100% style="display:flex; flex-direction: row; justify-content: left; align-items: center; gap: 5px;">
<a href="1-workshop.md"><img src="../assets/btn-previous.png" alt="Next" height="32"></a>
<a href="3-workshop.md"><img src="../assets/btn-next.png" alt="Next" height="32"></a>
</div>
